...
class Postprocess():
    ...
    def generate_html(self):
        ...
        html = self.Generate_html_merge_struct_pkl(merge_data, html_no_merge)
        self.thtml = html
        return html
    ...
    def Generate_html_merge_struct_pkl(self, merge_data, html_no_merge):
        """
        计算合并问题，并还原真正的表格结构
        html_no_merge(list:num_row*num_col):一行：:td/td,td/td......
        """
        len_col = len(self.point_col)
        len_row = len(self.point_row)
        html_pkl = deepcopy(html_no_merge)
        struct_pkl_text = deepcopy(html_no_merge)
        for id, merge in enumerate(merge_data):
            merge_cr = merge['merge_data']
            for cid in range(merge_cr[2], merge_cr[3]):
                num_col = min(len_col - 1, cid)
                num_row = min(len_row - 1, merge_cr[0])
                html_no_merge[num_row][num_col] = -1
                html_pkl[num_row][num_col] = -1
                struct_pkl_text[num_row][num_col] = -1
            for cid in range(merge_cr[2], merge_cr[3]):
                for rid in range(merge_cr[0], merge_cr[1]):
                    num_col = min(len_col - 1, cid)
                    num_row = min(len_row - 1, rid)
            ...
            if colspan > 1 and rowspan > 1:
                crdata = '<td ' + 'colspan="' + str(colspan) + '",rowspan="' + str(rowspan) + '">' + '</td>'
                crdata_2 = '<td, ' + 'colspan="' + str(colspan) + '", rowspan="' + str(rowspan) + '",>,' + '</td>'
                crdata_2 = ['<td',' rowspan="' + str(rowspan)+"'",' colspan="' + str(colspan) + "'",'>','</td>']
                crdata_2 = ['<td',' rowspan="' + str(rowspan)+'"',' colspan="' + str(colspan) + '"','>','</td>']
            elif rowspan > 1 and colspan <= 1:
                crdata = '<td ' + ',rowspan=\"' + str(rowspan) + '\">' + '</td>'
                crdata_2 = '<td, ' + ',rowspan=\"' + str(rowspan) + '\",>,' + '</td>'
                crdata_2 = ['<td',' rowspan="'+str(rowspan)+'"','>','</td>']

            elif rowspan <= 1 and colspan > 1:
                crdata = '<td ' + 'colspan=\"' + str(colspan) + '\">' + '</td>'
                crdata_2 = '<td, ' + 'colspan=\"' + str(colspan) + '\",>,' + '</td>'
                crdata_2 = ['<td', ' colspan="' + str(colspan) + '"', '>', '</td>']
            else:
                continue
            html_no_merge[merge_cr[0]][merge_cr[2]] = crdata
            html_pkl[merge_cr[0]][merge_cr[2]] = [rowspan,colspan]
            struct_pkl_text[merge_cr[0]][merge_cr[2]] = crdata_2
        for rid, tline in enumerate(html_no_merge):
            for cid in range(len(tline)):
                if -1 in tline:
                    tline.remove(-1)
                    ...
                    html_no_merge[rid] = tline
                    break
        for rid, tline in enumerate(struct_pkl_text):
            for cid in range(len(tline)):
                if -1 in tline:
                    tline.remove(-1)
                else:
                    struct_pkl_text[rid] = tline
                    break
        self.struct_pkl['text'] = struct_pkl_text
        self.struct_pkl['score'] = 0.99
        # self.struct_pkl['bbox'] = self.get_struct_pkl(html_pkl)
        self.struct_pkl['bbox'] = self.get_struct_pkl(html_pkl,self.struct_pkl['text'])
        html_merge = html_no_merge

        return html_merge
    def get_struct_pkl(self,html_pkl,struct_pkl):
        rows = self.point_row
        cols = self.point_col
        bboxs = []
        merge_list = []
        for merge_lines in struct_pkl:
            for merge_one in merge_lines:
                if type(merge_one).__name__=='list':
                    merge_list.append(merge_one)
        merge_have_used = 0
        wh_rate=1.0
        for rid in range(len(rows)-1):
            bbox = []
            bbox.append([0,0,0,0])
            bbox.append([0,0,0,0])
            for cid in range(len(cols)-1):
                box = html_pkl[rid][cid]
                if type(box).__name__=='list':
                    left = sum(cols[cid])/2
                    right = sum(cols[cid + box[1]])/2
                    up = sum(rows[rid])/2
                    down = sum(rows[rid + box[0]])/2
                    cp_x = (left + right) / 2
                    cp_y = (up + down) / 2
                    half_w_x = (right - left) / 2
                    half_w_y = (down - up) / 2
                    bbox.append([cp_x, cp_y, half_w_x*2*wh_rate, half_w_y*2*wh_rate])
                    for lens in range(len(merge_list[merge_have_used])-1):
                        bbox.append([0,0,0,0])
                    merge_have_used+=1
                elif type(box).__name__=='str':
                    left = sum(cols[cid])/2
                    right = sum(cols[cid+1])/2
                    up = sum(rows[rid])/2
                    down = sum(rows[rid+1])/2
                    cp_x = (left+right)/2
                    cp_y = (up+down)/2
                    half_w_x = (right-left)/2
                    half_w_y = (down-up)/2
                    bbox.append([cp_x,cp_y,half_w_x*2*wh_rate,half_w_y*2*wh_rate])
            bboxs.append(bbox)
        bboxs[-1].append([0,0,0,0])
        bboxs[-1].append([0,0,0,0])
        return bboxs
        # self.struct_pkl = {'text':"",
        #                    'score':0.99,
        #                    'bbox':bboxs
        #                    }
    ...
    def Get_score_out(self,thr_row,thr_col,merge_boxs,filename,imgshape,head_box,head=None):
        ...
        if len(self.point_col) <=1 or len(self.point_row) <=1:
            ...
            #                        file_name=filename,
            #                        delete_head=False)
        struct_bbox = self.struct_pkl['bbox']
        # bbox_head = struct_bbox[head_num]
        struct_bbox[head_num-1].append([0, 0, 0, 0])
        struct_bbox[head_num-1].append([0, 0, 0, 0])

        for bid in range(1,len(struct_bbox)):
            struct_bbox[0] = struct_bbox[0] + struct_bbox[bid]
        struct_bbox = np.array(struct_bbox[0])
        rate = max(imgshape)/800
        struct_bbox = struct_bbox*rate
        self.struct_pkl['bbox'] = struct_bbox
        struct_text = self.struct_pkl['text']
        struct_text = self.Add_head_struct_pkl(struct_text, head_num)
        self.struct_pkl['text'] = ','.join(struct_text)
        return_data = {
            'rows': self.point_row,
            'cols': self.point_col,
            'struct_pkl':self.struct_pkl
        }
    ...
    def Add_head_body(self, html, use_num=True, num=1, gt=None, no_merge=False):
        ...
        #     html_teds.append('</tbody>')
        return html_teds
    def Add_head_struct_pkl(self,html,headnum):
        html_teds = []
        num = headnum
        for id, tline in enumerate(html):
            line = []
            ...
            for t in tline:
                if type(t).__name__=='list':
                    line_data.append(','.join(t))
                else:
                    line_data.append(t)
            html_teds.append('<tr>,' + ','.join(line_data) + ',</tr>')
            if id == len(html) - 1 and num >= 0:
                html_teds.append('</tbody>')
            # html_teds.append(line)
        return html_teds
