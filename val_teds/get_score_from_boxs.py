...
class Postprocess():
    def __init__(self, thr_file, row_thr, col_thr,
                 merboxs, teds, gt_val_file, save_file,
                 min_width=4, iou=0.6,min_head_width=2,
                 head_thr=0.7):
        self.thr_file = thr_file
        self.row_thr = row_thr
        self.col_thr = col_thr
        self.merge_box = merboxs
        self.iou = iou
        self.data = []
        self.teds = teds
        self.score = []  # 所有的得分
    ...
    def Calcu_html(self, row_line=None, col_line=None, thr_merge=None, imgshape=None):
        # 得到行列坐标框
        ...
        if len(self.point_col) == 0 or len(self.point_row) == 0:
            return 0
        merge_data = self.get_merge_cr(point_row, point_col, self.merge_box)  # 获得合并信息，未进行iou判断
        # if len(merge_data)!=0:
    ...
    def Calcu_html_from_col_row_box(self, row_boxs=None, col_boxs=None, merge_boxs=None, imgshape=None):
        point_row = []
        point_col = []
        for id,row in enumerate(row_boxs):
            up = min(row[1],row[3])
            down = max(row[1],row[3])
            point_row.append([int(up),int(down)])
        for id,col in enumerate(col_boxs):
            left = min(col[0],col[2])
            right = max(col[0],col[2])
            point_col.append([int(left),int(right)])
        point_row = np.array(point_row)
        index = point_row[:,0].argsort()
        point_row = point_row[index]
        point_row = point_row.tolist()

        point_col = np.array(point_col)
        index = point_col[:, 0].argsort()
        point_col = point_col[index]
        point_col = point_col.tolist()
        # merge_boxs = merge_boxs.tolist()
    ...
    def Generate_html_merge(self, merge_data, html_no_merge):
        ...
        for id, merge in enumerate(merge_data):
            ...
            for cid in range(merge_cr[2], merge_cr[3]):
                html_no_merge[merge_cr[0]][cid] = -1
            for cid in range(merge_cr[2], merge_cr[3]):
                for rid in range(merge_cr[0], merge_cr[1]):
                    html_no_merge[rid][cid] = -1
    ...
    def Get_score_out(self,row_boxs,col_boxs,merge_boxs,head_box,filename,imgshape,head=None):
        # thr_merge = cv2.resize(thr_merge, dsize=(0, 0), fy=2, fx=2, interpolation=cv2.INTER_AREA)
        # if thr['filename'] == 'PMC1557538_002_01.png':
        #     # if thr['filename']=='PMC1557538_002_01.png':
        #     print(thr)
            ...
            # cv2.waitKey(0)
        self.init()
        self.Calcu_html_from_col_row_box(row_boxs=row_boxs,col_boxs=col_boxs, merge_boxs=merge_boxs, imgshape=imgshape)
        if len(self.point_col) == 0 or len(self.point_row) == 0:
            score = 0
        else:
            self.get_real_merge()  # 获得真实merge合并的框<做了iou判断>
    ...
    def get_head_num_from_box(self,head_points,row_points):
        ...
        if len(head_points)<1:
            return 1
        head_points = head_points[0].reshape(1,-1).tolist()
        if type(head_points[0]).__name__=="list" and len(head_points[0])>=4:
            head_points = head_points[0]
        elif len(head_points)>=4:
