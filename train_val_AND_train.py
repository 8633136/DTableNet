...
def main(args):
    ...
    # writer.add_scalar('TEDS', teds, args.start_epoch-1)
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default='../voc_pubtabnet_new2', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=2, type=int, help='num_classes')
    # 文件保存地址
    parser.add_argument('--output-dir', default='./work710_convt', help='path where to save')
    # 若需要接着上次训练，则指定上次训练保存权重文件地址
    parser.add_argument('--resume', default='./710_serve_test/resNetFpn-model-20.pth', type=str, help='resume from checkpoint')
    # 指定接着从哪个epoch数开始训练
