...
def main():
    ...
    box_thresh = 0.9
    # save low score img
    save_low_score_folder = "./low_score_convt_epoch22_YOLO"
    low_score_thr = 0.8
    low_score_folder_list=['0.8-0.6','0.6-0.4','0.4-0.0']
    low_score_thr_list = [0.6,0.4,0.0]
    save_file_for_yolo = os.path.join(save_low_score_folder,'cr_data.json')
    save_cr_data={}
        ...
        with torch.no_grad():
            ...
            for one_id,pred in enumerate(p2_c):
                ...
                score,point_row,point_col,headnum,other_data = line_post.Get_score_out(thr_row=row,thr_col=col,merge_boxs=merge_boxs,
                                                ...
                                                head_box=head_box)
                save_cr_data[filename_list[one_id]] = other_data
    ...
    save_cr_data = json.dumps(save_cr_data)
    f = open(save_file_for_yolo,mode='w',encoding='utf-8')
    f.write(save_cr_data)
    f.close()
    print('ave_score:', sum(score_list) / len(score_list))
