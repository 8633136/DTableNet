...
def main():
    ...
    # weights_path = "./work720_low_score_zpeight/resNetFpn-model-35.pth"
    weights_path = "./725_serve_test_convt/ori/resNetFpn-model-28.pth"
    weights_path = "./731_serve_test_convt/resNetFpn-model-34.pth"
    ...
    line_post = Line_Postprocess(
                            ...
                            teds=TEDS(n_jobs=1),
                            gt_val_file=gt_val_file,
                            min_width_row=8,min_width_col=8,
                            save_file='./Val_data.json',
                            iou=0.7)
    box_thresh = 0.9
    # save low score img
    save_low_score_folder = "./low_score_train_convt_all_epoch34"
    low_score_thr = 1.0
    ...
    VOC_root = r"/home/li/Desktop/disk2/datasets/pubtabnet/voc_pubtabnet_dence"
    val_dataset = VOCDataSet(VOC_root, data_transform, "val.txt")
    ...
    with torch.no_grad():
        for images,targets in tqdm(val_data_loader):
            ...
            predictions, _, p2_r, p2_c = model(images)
            for one_id, pred in enumerate(p2_c):
                ...
                score, point_row, point_col, headnum, other_data = line_post.Get_score_out(thr_row=row, thr_col=col,
                                                                                           merge_boxs=merge_boxs,
                                                                                           filename=targets[one_id]['file_name'],
                                                                                           imgshape=[],
                                                                                           head_box=head_box)
