...
def main():
    ...
    weights_path = "./725_serve_test_convt/ori/resNetFpn-model-27.pth"
    ...
    box_thresh = 0.9
    # save low score img
    save_low_score_folder = "./low_score_all_for_train_727"
    save_img_txt = os.path.join(save_low_score_folder,'low_score.txt')
    save_score_file = os.path.join(save_low_score_folder,'train_score_crm_data.json')
    save_json = {}
    img_name_list = []
    low_score_thr = 1.0
        ...
        with torch.no_grad():
            ...
            for one_id,pred in enumerate(p2_c):
                ...
                score,point_row,point_col,headnum,_ = line_post.Get_score_out(thr_row=row,thr_col=col,merge_boxs=merge_boxs,
                                                ...
                                                head_box=head_box)
                save_json[filename_list[one_id]] = {
                    'score':score,
                    'num_head':headnum,
                    'num_col':len(point_col),
                    'num_row':len(point_row)
                }
                if score<low_score_thr:
                    low_score_list.append(score)
                    img_name_list.append(filename_list[one_id].replace('.png',''))
                ...
                if len(score_list)%10000==1:
                    print('ave_score:',sum(score_list)/len(score_list))
                    f = open(save_img_txt, mode='w', encoding='utf-8')
                    f.write('\n'.join(img_name_list))
                    f.close()
                    f2 = open(save_score_file,'w',encoding='utf-8')
                    f2.write(json.dumps(save_json))
            filename_list = []
            continue
