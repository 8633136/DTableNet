...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales,
        ...
        ws = (scales[:, 0] * aspect_ratios ).view(-1)
        hs = (scales[:, 1] * aspect_ratios).view(-1)
        ...
        # a=w_ratios[:, None]
        # b=scales[:,0]
        # ws = (w_ratios[:, None] * scales[:,0]).view(-1)
        # hs = (h_ratios[:, None] * scales[:,1]).view(-1)
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)  #num_anchors:12
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def forward(self,
        ...
        a=pred_bbox_deltas.detach()  # a=12*b
        b=anchors
