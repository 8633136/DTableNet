...
from val_teds.get_score_line_V3 import Postprocess as Line_Postprocess
...
def main():
    ...
    # exit(0)
    # load train weights
    # weights_path = "./work529_on525/resNetFpn-model-1.pth"
    # weights_path = "./work604_2class/resNetFpn-model-6.pth"
    ...
    weights_path = "./725_serve_test_convt/ori/resNetFpn-model-28.pth"
    # weights_path = "./731_serve_test_convt/resNetFpn-model-36.pth"
    weights_path = "./A824_serve_test/resNetFpn-model-17.pth"
    ...
    # img_folder = "/home/li/Desktop/disk2/datasets/pubtabnet/dence_img_4d5_no800/dence"
    # img_folder = "/home/li/Desktop/disk2/datasets/pubtabnet/pubtabnet_sim_com/pubtabnet_sim_com/complex"
    ...
    line_post = Line_Postprocess(
                            ...
                            DBPost_merge=[],
                            teds=TEDS(n_jobs=1,structure_only=True),
                            gt_val_file=gt_val_file,
    ...
    box_thresh = 0.9
    # save low score img
    save_low_score_folder = "./A824_R18_epoch17_save_pkl"
    save_img_txt = os.path.join(save_low_score_folder,'low_score.txt')
    save_struct_pkl_file = os.path.join(save_low_score_folder,'struct_TM_R18_epoch17_box0d9.pkl')
    save_struct_dict = {}
    img_name_list = []
    low_score_thr = 1.0
    ...
    for id,img in enumerate(img_list):
        ...
        if num_now==0:
            ...
            file_shape_list.append(traget['scnn_ori_shape'])
            num_now +=1
        ...
        with torch.no_grad():
            ...
            # row = (255 * p2_r[0]).to('cpu', torch.int16).detach().numpy()
            # col = (255 * p2_c[0]).to('cpu', torch.int16).detach().numpy()
            # predictions2,_,p2_r2,p2_c2 = model2(img_batch.to(device),targets=[])
            ...
            for one_id,pred in enumerate(p2_c):
                ...
                # if len(row_boxs)<=0 or len(col_boxs)<=0:
                #     score = 0
                #     # print("---------------row==0 or col==0")
                # else:
                #     pass
                #     merge_boxs = []
                #     if len(head_box)>1:
                ...
                #                        head_box=head_box,filename=filename_list[one_id],imgshape=[])
                # merge_boxs = []
                row = (255 * p2_r[one_id]).to('cpu', torch.int16).detach().numpy()
                col = (255 * p2_c[one_id]).to('cpu', torch.int16).detach().numpy()
                score,point_row,point_col,headnum,other_data = line_post.Get_score_out(thr_row=row,thr_col=col,merge_boxs=merge_boxs,
                                                filename=filename_list[one_id],imgshape=file_shape_list[one_id],
                                                head_box=head_box)
                struct_data = other_data['struct_pkl']
                save_struct_dict[filename_list[one_id]] = struct_data
                if score<low_score_thr and True:
                ...
                if len(score_list)%100==1:
                    ...
                    f = open(save_struct_pkl_file,'wb')
                    pickle.dump(save_struct_dict,f)
                    f.close()
            filename_list = []
            file_shape_list = []
            continue
        # break
    f = open(save_img_txt,mode='w',encoding='utf-8')
    f.write('\n'.join(img_name_list))
    f.close()
    f = open(save_struct_pkl_file, 'wb')
    pickle.dump(save_struct_dict, f)
