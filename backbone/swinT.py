...
class SwinT(nn.Sequential):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            x = self.cnn(x)
        return x
class My_Make_SwinT_Layer(nn.Sequential):
    def __init__(self, in_channels, out_channels, input_resolution, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, dropout=0.,
                 attention_dropout=0., droppath=0., downsample=False,num_layer=1):
        super(My_Make_SwinT_Layer,self).__init__()
        self.my_layer = []
        for id in range(num_layer-1):
            self.my_layer.append(
                SwinT(in_channels, out_channels, input_resolution, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, dropout=0.,
                 attention_dropout=0., droppath=0., downsample=False))
        self.my_layer.append(SwinT(out_channels, out_channels, input_resolution, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, dropout=0.,
                 attention_dropout=0., droppath=0., downsample=False))
        return tnn.Sequential(*self.my_layer)
    def forward(self, x):
        for layer in self.my_layer:
            x = layer(x)