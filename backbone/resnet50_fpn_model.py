...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes: int, planes: int, stride: int = 1, downsample=None,norm_layer=None):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=False, norm_layer=None):
        ...
        for m in self.modules():   # 初始化模型
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        self.inplanes1 = 64
        self.inplanes2 = 512
        self.ca = ChannelAttention(self.inplanes1)
        self.sa = SpatialAttention()
        # 网络的卷积层的最后一层加入注意力机制
        self.ca1 = ChannelAttention(self.inplanes2)
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    # freeze layers
    #遍历 ResNet50 backbone 网络中的所有参数，即 resnet_backbone.named_parameters()。
    #对于每个参数，检查其名称是否以 layers_to_train 列表中的任何字符串开头。
    #如果参数名称不以 layers_to_train 列表中任何字符串开头，则将其 requires_grad 属性设置为 False，
    # 即冻结该参数，不参与训练。
    ...
    if returned_layers is None:
        returned_layers = [1, 2, 3, 4]  # torch.Size([1, 256, 200, 200]) torch.Size([1, 256, 100, 100]) torch.Size([1, 256, 50, 50]) torch.Size([1, 256, 25, 25])
    # 返回的特征层个数肯定大于0小于5
...
def resnet18_fpn_backbone(pretrain_path="",
    ...
    # select layers that wont be frozen
    assert 0 <= trainable_layers <= 5
    # layers_to_train = ['sa1','ca1','layer4', 'layer3', 'layer2', 'layer1', 'sa','ca','conv1'][:trainable_layers+4]
    layers_to_train = ['layer4', 'layer3', 'layer2', 'layer1','conv1'][:trainable_layers]
